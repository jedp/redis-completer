  ...
  addCompletions(phrase, id, score, callback) {
    ...
    words.forEach((word) => {
      ...
      for (i = 1; i <= word.length; i++) {
        ...
        redis.zadd(this.zkey_autocomplete, score, character, callback);
      }

      // Insert the word with the ending character in the namespace:autocomplete
      redis.zadd(this.zkey_autocomplete, score, `${word}*`, callback);

      // Insert the doc (namespace:autocomplete:docs:word) that contains the whole phrase
      redis.zadd(`${this.zkey_docs_prefix}${word}`, score, phraseToStore, callback);
    });
  }
